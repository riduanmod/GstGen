...
app = FastAPI(
    ...
    description="Generates and fully activates a Free Fire guest account based on OB52 Fixed Logic.",
    version="2.0.0"
)
...
def build_activation_payload(jwt_token: str, access_token: str) -> bytes:
    try:
        ...
        padding = 4 - len(payload_part) % 4
        if padding != 4: payload_part += '=' * padding
        decoded = json.loads(base64.urlsafe_b64decode(payload_part).decode('utf-8'))
...
def generate_and_activate_account(
    name: str = Query(..., description="Exact desired account name"),
    ...
    client_host = "client.ind.freefiremobile.com" if region_code == "IND" else "clientbp.ggblueshark.com"
    ...
    try:
        ...
        # UPDATED ERROR MESSAGE FOR DUPLICATE NAME ISSUE
        if not jwt_token or account_id == "N/A": 
            raise ValueError("Major Login Failed. Reason: The account name you provided is already taken by another player. Free Fire requires a completely UNIQUE name. Please try again with a unique name.")
        ...
        # --- 6. ACCOUNT ACTIVATION (GetLoginData) ---
        ...
        if activation_payload:
            ...
            try:
                res_act = req_session.post(f"https://{client_host}/GetLoginData", headers=act_headers, data=activation_payload, verify=False, timeout=10)
                if res_act.status_code == 200:
                    is_activated = True
            except Exception as e:
                logging.error(f"Activation request failed: {e}")

        return AccountResponse(
            status="success",
            message="Account generated and successfully activated." if is_activated else "Account generated but activation failed.",
        ...
        raise HTTPException(status_code=500, detail=f"Account Generation/Activation Failed: {str(e)}")
