...
# Vercel Serverless Import Fix: Ensure current directory is in path
...
# --- Try loading Protobuf for MajorLoginRes ---
try:
    import MajorLoginRes_pb2
    PROTOBUF_AVAILABLE = True
    logging.info("✅ MajorLoginRes_pb2 imported successfully")
except ImportError as e:
    PROTOBUF_AVAILABLE = False
    logging.warning(f"⚠️ MajorLoginRes_pb2 not found. Will use string parsing fallback for JWT. Error: {e}")
...
def build_activation_payload(jwt_token: str, access_token: str) -> bytes:
    try:
        ...
        logging.error(f"Error building activation payload: {e}")
        return None
...
def generate_and_activate_account(
    ...
    for attempt in range(max_attempts):
        logging.info(f"🚀 Attempt {attempt+1}: Generating & Activating account for {final_account_name} in {region_code}")
        try:
            # --- 1. GUEST REGISTER ---
            ...
            res_reg = requests.post("https://100067.connect.garena.com/oauth/guest/register", headers=reg_headers, data=data, verify=False, timeout=10)
            if 'uid' not in res_reg.json(): continue
            ...
            res_tok = requests.post("https://100067.connect.garena.com/oauth/guest/token/grant", headers=reg_headers, data=body, verify=False, timeout=10)
            tok_data = res_tok.json()
            if 'open_id' not in tok_data: continue
            ...
            requests.post(major_url, headers=headers_major, data=enc_payload_mr, verify=False, timeout=10)
            ...
            res_ml = requests.post(f"https://{config['login_host']}/MajorLogin", headers=headers_major, data=enc_ml_data, verify=False, timeout=10)
            
            if res_ml.status_code != 200: continue
            ...
            if region_code != "BR":
                ...
                requests.post(f"https://{config['login_host']}/ChooseRegion", headers=headers_choose, data=bytes.fromhex(enc_region), verify=False, timeout=10)

            # --- 6. GET LOGIN DATA (FINAL ACTIVATION) ---
            activation_payload = build_activation_payload(jwt_token, access_token)
            if not activation_payload: continue
            ...
            headers_act = {
                ...
                'Host': config['client_host'], 'Expect': '100-continue'
            }
            
            res_act = requests.post(f"https://{config['client_host']}/GetLoginData", headers=headers_act, data=activation_payload, verify=False, timeout=10)
            
            # --- 7. CHECK ACTIVATION STATUS ---
            if res_act.status_code == 200:
                logging.info(f"✅ Account {final_account_name} successfully activated!")
                
                return AccountResponse(
                    status="success",
                    message="Account has been generated and successfully activated.",
                    is_active=True,
                ...
                logging.warning(f"❌ GetLoginData failed with {res_act.status_code}. Retrying...")

        except Exception as e:
            logging.error(f"Attempt {attempt+1} failed due to error: {str(e)}")
            time.sleep(1) 

    raise HTTPException(status_code=500, detail="Failed to generate or activate the account after multiple attempts.")
